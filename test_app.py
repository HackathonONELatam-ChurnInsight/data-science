...
def test_predict_churn_success_churn():
    ...
    with patch("app.model") as mock_model:
        ...
        # predict devuelve un array, ej: [1]
        mock_model.predict.return_value = [1]
        # predict_proba devuelve array de arrays [[prob_0, prob_1]]
...
Spain,Male,35,600,1000.0,50000.0,3,2,4,0,1,0,1002,Juan"""
    ...
    with patch("app.model") as mock_model:
        # Configurar mocks devolviendo ndarrays de numpy para soportar slicing [:, 1]
        # 2 filas -> 2 predicciones
        mock_model.predict.return_value = np.array([1, 0])
        mock_model.predict_proba.return_value = np.array([[0.15, 0.85], [0.95, 0.05]])
